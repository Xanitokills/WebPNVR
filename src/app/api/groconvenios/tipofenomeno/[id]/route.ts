...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const requestQuery = pool.request()
      .input("id_tipo_fenomeno", sql.Int, id)
    ...
    const updatedTipoFenomeno = {
      id_tipo_fenomeno: id,
      descripcion: body.descripcion,
      ...(body.estado !== undefined && { estado: body.estado }),
    };
    return NextResponse.json(updatedTipoFenomeno);
  } catch (error) {
    console.error("Error en la consulta PUT:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo actualizar el tipo de fenómeno", details: errorMessage },
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ message: "Tipo de fenómeno eliminado" });
  } catch (error) {
    console.error("Error en la consulta DELETE:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo eliminar el tipo de fenómeno", details: errorMessage },
      { status: errorMessage.includes("Faltan las siguientes variables") ? 400 : 500 }
    );
  }
}