...
export async function GET() {
  try {
    ...
    const resultado = await pool
      .request()
      .query("SELECT id_tipo_intervencion, descripcion, estado FROM [dbo].[PNVR_Tipo_Intervencion]");
    return NextResponse.json(resultado.recordset);
  } catch (error) {
    console.error("Error en la consulta GET:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudieron obtener los datos de los tipos de intervención", details: errorMessage },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await pool
      ...
      .query(
        "INSERT INTO [dbo].[PNVR_Tipo_Intervencion] (descripcion, estado) VALUES (@descripcion, @estado); SELECT SCOPE_IDENTITY() as id_tipo_intervencion"
      );

    const newId = result.recordset[0].id_tipo_intervencion;
    const newTipoIntervencion = {
      id_tipo_intervencion: newId,
      descripcion: body.descripcion,
      estado: body.estado !== undefined ? body.estado : null,
    };
    return NextResponse.json(newTipoIntervencion, { status: 201 });
  } catch (error) {
    console.error("Error en la consulta POST:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo crear el tipo de intervención", details: errorMessage },
      { status: errorMessage.includes("Faltan las siguientes variables") ? 400 : 500 }
    );
  }
}