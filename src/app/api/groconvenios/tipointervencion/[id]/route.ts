...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const requestQuery = pool.request()
      .input("id_tipo_intervencion", sql.Int, id)
    ...
    const query = `
      UPDATE [dbo].[PNVR_Tipo_Intervencion]
    ...
    if (result.rowsAffected[0] === 0) {
      return NextResponse.json({ error: "Tipo de intervención no encontrado" }, { status: 404 });
    }

    const updatedTipoIntervencion = {
      id_tipo_intervencion: id,
      descripcion: body.descripcion,
      ...(body.estado !== undefined && { estado: body.estado }),
    };
    return NextResponse.json(updatedTipoIntervencion);
  } catch (error) {
    console.error("Error en la consulta PUT:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo actualizar el tipo de intervención", details: errorMessage },
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM [dbo].[PNVR_Tipo_Intervencion] WHERE id_tipo_intervencion = @id");

    if (result.rowsAffected[0] === 0) {
      return NextResponse.json({ error: "Tipo de intervención no encontrado" }, { status: 404 });
    }

    return NextResponse.json({ message: "Tipo de intervención eliminado" });
  } catch (error) {
    console.error("Error en la consulta DELETE:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo eliminar el tipo de intervención", details: errorMessage },
      { status: errorMessage.includes("Faltan las siguientes variables") ? 400 : 500 }
    );
  }
}