...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "No se pudieron obtener los datos de los tipos de material", details: errorMessage },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newTipoMaterial = {
      id_tipo_material: newId,
      descripcion: body.descripcion,
      estado: body.estado !== undefined ? body.estado : null,
    };
    return NextResponse.json(newTipoMaterial, { status: 201 });
  } catch (error) {
    console.error("Error en la consulta POST:", error);
    const errorMessage = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudo crear el tipo de material", details: errorMessage },
      { status: errorMessage.includes("Faltan las siguientes variables") ? 400 : 500 }
    );
  }
}